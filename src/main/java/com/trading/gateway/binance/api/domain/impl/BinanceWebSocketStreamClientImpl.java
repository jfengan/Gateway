...
import com.trading.gateway.utils.websocket.*;
...
public class BinanceWebSocketStreamClientImpl implements SubscriptionClient {

    private final SubscriptionOptions options;
    private WatchDog watchDog;

    private final BinanceWebsocketRequestImpl requestImpl;

    private final List<BinanceWebSocketConnection> connections = new LinkedList<>();

    public BinanceWebSocketStreamClientImpl(SubscriptionOptions options) {
        this.watchDog = null;
        this.options = Objects.requireNonNull(options);

        this.requestImpl = new BinanceWebsocketRequestImpl();
    }


    private <T> void createConnection(WebsocketRequest<T> request, boolean autoClose) {
        if (watchDog == null) {
            watchDog = new WatchDog(options);
        }
        BinanceWebSocketConnection connection = new BinanceWebSocketConnection(request, watchDog, autoClose);
        if (autoClose == false) {
    ...
    public void unsubscribeAll() {
        for (BinanceWebSocketConnection connection : connections) {
